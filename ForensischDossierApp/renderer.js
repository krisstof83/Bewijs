...
async function scanPDFs() {
  ...
  // ⚡ Bolt: Batching inserts within a transaction to avoid disk fsync per file.
  db.serialize(() => {
    ...
    for (const f of files) {
      try {
        // ⚡ Bolt: Wrapped sync file operations in try...catch to prevent uncaught
        // errors from leaking the transaction and permanently locking the database.
