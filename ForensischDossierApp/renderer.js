...
async function scanPDFs() {
  ...
  db.serialize(() => {
    // ⚡ Bolt Optimization: Batching SQLite inserts in a transaction
    // prevents disk fsync bottlenecks for each inserted file, leading to
    // significant performance gains (up to 100x faster for bulk imports).
