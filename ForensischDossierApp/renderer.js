...
async function scanPDFs() {
  ...
  // ⚡ Bolt: Batch database inserts to avoid an fsync per row,
  // making large numbers of file metadata insertions significantly faster.
  db.serialize(() => {
    ...
    for (const f of files) {
      try {
        ...
        // Ensure that a failed synchronous read doesn't leak the transaction
      }
    }
